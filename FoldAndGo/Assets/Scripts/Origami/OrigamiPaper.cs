...
public class OrigamiPaper : MonoBehaviour {
    ...
    private void doFold(OrigamiStep step) {
    ...
    IEnumerator doAnimatedFold(OrigamiStep step, float duration, bool reverse = false) {
        ...
        float lastRotation = origamiData.DEFAULT_FOLD_ROTATION - totalRotation;
    ...
    private void undoFold(OrigamiStep step) {
        Vector3 pivot = calculateFoldingMiddlePoint(step.foldingPointsAxeIndex[0], step.foldingPointsAxeIndex[1]);
        Vector3 axis = calculateAxis(step.foldingPointsAxeIndex[0], pivot);
    ...
    public void nextStep() {
        ...
        Debug.Log("nextStep stepsList.Count = " + origamiData.stepsList.Count);
...
//    public Vector3[] vertices;
//    int[] triangles;
//    List<OrigamiStep> stepsList = new List<OrigamiStep>();
//    int currentStepIndex = -1;
//    int nbOfSteps = 4;
//    OrigamiStep step1;
//    OrigamiStep step2;
